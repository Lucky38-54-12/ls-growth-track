...
import { createDocFromMarkedTextWithPhotos } from "./googleDocs";
...
export async function generateAgreementText(input: AgreementInput): Promise<{ title: string; body: string }> {
...
export async function generateAgreementDoc(input: AgreementInput, photosFolderUrl?: string): Promise<string> {
  const { title, body } = await generateAgreementText(input);
  return createDocFromMarkedTextWithPhotos(title, body, photosFolderUrl);
}
