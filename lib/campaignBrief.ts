...
COPYWRITING CRAFT — headline, primaryText and hook are the actual ad copy Lucky could paste into Meta Ads Manager, not fields to fill in. Write like a specific person talking to a specific homeowner: ban "we help [persona] [verb] their [thing]," "At [Business], we...," stacked adjectives ("outdated, tired, run-down"), and closing every ad with the same "no pressure, no obligation, tap below" line. If two ads in this same set share a sentence structure or closer, rewrite one — that's the tell of a template rather than real writing. A real number, a named local detail, or a concrete outcome beats "quality," "trusted," or "amazing" every time — pull specifics only from what's actually true about this client/service, never invent one. Primary text should read as direct-response copy with a clear spine (real problem in the reader's words → one reason to believe → one specific offer/CTA), not a longer restatement of the headline. Before finalizing, reread each headline/hook/primaryText and cut anything generic enough to paste into any other local trade business's ad with a find-and-replace on the name — if it still works with the name swapped, it isn't specific enough yet.
