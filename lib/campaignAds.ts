...
- referenceLinks: use the web_search tool to find 2-3 real VIDEO links Lucky can actually watch for inspiration on this ad's creative — real ad examples or reference footage matching the creativeDirection, not written content. Every link must point directly at a video: a specific YouTube video/Short, Instagram Reel/post, TikTok video, or a specific clip page on a stock footage site (Pexels/Pixabay/Coverr/Storyblocks video pages). NEVER return a blog post, article, "how to run Facebook ads" guide, marketing-tips page, or any other link that isn't itself a video — those are useless to him and worse than returning nothing. Search with video-specific terms (e.g. site:youtube.com, site:instagram.com/reel, "deck build time lapse", "process video [trade]") rather than generic advice queries. If you can't find real video links for an ad after searching, return an empty list for it rather than substituting an article.
