...
function ConnectPageInner() {
  ...
  function loadClient() {
    fetch(`/api/lead-qual/public/${clientId}`)
      .then((r) => r.json())
      .then((body) => {
        if (body.error) setNotFound(true);
        else setClient(body);
      })
  ...
  if (notFound) {
    return (
      <Centered>
        <p style={{ fontSize: 14, color: L.muted }}>That link isn&apos;t valid. Ask Lucky at LS Growth for a fresh one.</p>
      </Centered>
    );
  }
  ...
  if (!client) {
    return (
      <Centered>
        <p style={{ fontSize: 14, color: L.muted }}>Loading…</p>
      </Centered>
    );
  }
  ...
  return (
    <Centered>
      <div style={{ width: "100%", maxWidth: 460 }}>
        <p style={{ fontSize: 12, fontWeight: 700, color: "var(--red)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>
          LS Growth
        </p>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: L.text, marginBottom: 4 }}>Connect {client.name}</h1>
        <p style={{ fontSize: 14, color: L.muted, marginBottom: 28 }}>
          Two quick connections so leads book straight onto your calendar. You&apos;ll log into your own Google and Facebook accounts, nothing is shared with us beyond what you approve on their screens.
        </p>
        ...
        {fbPending && fbPages.length > 0 ? (
          <div style={{ background: L.surface, border: `1px solid ${L.border}`, borderRadius: 10, padding: 16, marginBottom: 14 }}>
        ...
        {calendarDone && facebookDone && (
          <p style={{ fontSize: 13, color: "#15803d", fontWeight: 600, marginTop: 8 }}>
