...
export default function ConnectFlow({ routeClientId }: { routeClientId?: string }) {
  ...
  // Coming back from Google/Facebook's consent screen is a full page
  // redirect (loses any in-memory state), so that trip skips straight past
  // the paste-and-verify gate — the link was already proven genuine the
  // first time through, re-asking would just be friction.
  const arrivingMidFlow = !!(routeClientId && (calendarConnected || calendarError || fbError || fbPending));
  ...
  const [step, setStep] = useState<"facebook" | "calendar" | "ads" | "login">(() => {
  ...
  async function handleChoosePage(pageId: string) {
    ...
    const res = await fetch("/api/lead-qual/oauth/facebook/confirm", {
      ...
      body: JSON.stringify({ pendingId: fbPending, clientId: resolvedClientId, pageId }),
    });
    ...
    <div className="connect-shell" style={{ minHeight: "100vh", display: "flex", background: "#fff" }}>
      <style>{`
        ...
        @media (max-width: 780px) {
          ...
          .connect-brand-logo { height: 48px; }
      ...
      <div className="connect-brand" style={{ display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        <img src="/logo-wide.png" alt="LS Growth" className="connect-brand-logo" style={{ width: "auto", maxWidth: "100%", objectFit: "contain", alignSelf: "flex-start" }} />
