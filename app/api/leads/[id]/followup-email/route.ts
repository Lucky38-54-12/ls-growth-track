...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const notesBlock = lead.notes?.trim() ? `NOTES:\n${lead.notes.trim()}` : "";

  const prompt = `You are writing a re-engagement email for Lucky at LS Growth Agency. LS Growth gets trade businesses more booked jobs — specific jobs, real revenue, never describe the mechanism or process.
...
Write a short re-engagement follow-up email. Rules:
- Read the thread above and write an email that makes sense given what happened last — if they replied, acknowledge it; if they went cold, re-open naturally
